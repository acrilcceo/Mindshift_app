...
const RitualOverlay: React.FC<RitualOverlayProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center transition-opacity duration-1000 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      {/* Ambient Background */}
      <div className={`absolute inset-0 bg-black/90 backdrop-blur-xl transition-all duration-1000 ${auraStyles.background}`} />

      {/* Content Container */}
      <div className="relative z-10 w-full max-w-md p-8 text-center text-white">
        
        {/* Step 1: Intro */}
        {step === 'intro' && (
          <div className="animate-fade-in flex flex-col items-center gap-4">
            <h2 className="text-3xl font-serif tracking-wide text-white/90">Pause.</h2>
            <p className="text-white/60 text-lg">Disconnect to reconnect.</p>
          </div>
        )}
          ...
          <div className="flex flex-col items-center gap-8 animate-fade-in">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-2xl font-light tracking-widest text-white/80">Breathe</span>
              </div>
            </div>
            <p className="text-white/50 text-sm tracking-wide">Inhale... Exhale...</p>
          </div>
        )}
        ...
        {step === 'intent' && (
          <div className="animate-fade-in w-full">
            <h3 className="text-2xl font-serif mb-8 text-white/90">Set your intention</h3>
            <div className="flex flex-col gap-4">
              {['Clarity', 'Peace', 'Strength', 'Gratitude'].map((intent) => (
                <button
                  key={intent}
                  onClick={() => handleIntentSelect(intent)}
                  className="w-full py-4 px-6 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-accent-primary/50 transition-all duration-300 text-lg tracking-wide text-white/80 hover:text-white"
                >
                  {intent}
                </button>
              ))}
