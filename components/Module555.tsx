...
const Module555: React.FC<Module555Props> = ({ state, onUpdate }) => {
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="text-center">
          <h2 className="text-2xl font-serif text-heading-primary">5-5-5 Ritual</h2>
          <p className="text-sm mt-1 text-body-main">Deep neural imprinting via massive repetition.</p>
        </div>
        <div className="glass-card card-elevated p-8 space-y-6 rounded-2xl">
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-widest text-helper font-bold">
              Your Core Intention
            </label>
            <textarea
              ...
              className="input-soft rounded-2xl min-h-[96px]"
            />
          </div>
          <button
            onClick={startModule}
            className="w-full py-4 rounded-2xl btn-ritual-amber"
          >
            Commence 5-Day Ritual
          </button>
          <div className="text-[10px] text-helper text-center leading-relaxed italic">
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-left-4 duration-500">
      <div className="text-center">
        <h2 className="text-2xl font-serif text-heading-primary">Day {currentModule.currentDay} / 5</h2>
        <div className="text-lg font-medium mt-2 px-4 italic text-body-main">"{currentModule.affirmation}"</div>
      </div>
      ...
      <div className="flex justify-center items-center py-10">
        <button
          ...
          className="w-40 h-40 rounded-full glass-card flex flex-col items-center justify-center ring-4 ring-amber-500/20 active:scale-95 transition-transform bg-white"
        >
          <span className="text-5xl font-bold text-heading-secondary">{dailyProgress}</span>
          <span className="text-[10px] uppercase tracking-widest text-helper mt-2">Repetitions</span>
        </button>
      </div>

      <div className="glass-card card-elevated p-6 rounded-2xl">
              ...
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold ${
                  currentModule.currentDay >= d
                    ? 'bg-amber-500 text-heading-primary'
                    : 'bg-[#EEF2F6] text-helper'
                }`}
              >
                {d}
              </div>
              <div className="h-1 w-full bg-[#E3EAF2] mt-2 rounded-full overflow-hidden">
                <div 
        ...
        {currentModule.isCompleted ? (
          <div className="text-center text-sm font-semibold uppercase tracking-widest mt-4 text-heading-secondary">
            Ritual Complete ✨ The seed is sown.
          </div>
        ) : (
          <div className="text-center text-xs italic text-helper">
